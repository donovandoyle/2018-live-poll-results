...
```{r}
joined <- left_join(results, polls_clean, by = "state_district")
...
```{r}
test <- polls %>%
  select(source, response, genballot, hdem_fav, hrep_fav, ager, educ, gender, race_eth) %>%
  mutate(neat = str_split(source, "-")) %>%
  separate(neat, into = c("c", "year", "live", "poll", "results", "master", "data", "elections", "poll_again", "state_district", "wave")) %>%
  select(state_district, response, wave, ager, educ, gender, race_eth, genballot, hdem_fav, hrep_fav) %>%
  separate(state_district, into = c("state", "district"), sep = 2) %>%
  filter(wave == "3") %>%
  filter(race_eth != "[DO NOT READ] Don't know/Refused") %>%
  filter(educ != "[DO NOT READ] Don't know/Refused") %>%
